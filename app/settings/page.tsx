...
export default function SettingsPage() {
  ...
  const villes = ['Casablanca', 'Rabat', 'Marrakech', 'Fès', 'Tanger', 'Agadir', 'Meknès', 'Oujda', 'Kenitra', 'Autre'];
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <h2 className="font-semibold text-gray-700 mb-4 text-blue-600">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={company.banque} onChange={e => setCompany({...company, banque: e.target.value})} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-blue-400">
                  <option value="">Choisir...</option>
                  {['Attijariwafa Bank', 'CIH Bank', 'BMCE Bank', 'BMCI', 'Banque Populaire', 'Société Générale', 'CFG Bank', 'Al Barid Bank'].map(b => <option key={b} value={b}>{b}</option>)}
                </select>
              </div>
              <div>
